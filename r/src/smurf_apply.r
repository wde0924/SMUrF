...
#' rename as smurf_apply(), originated from stilt/stilt_apply
#' add @param jobname to separate the log files 

smurf_apply <- function(FUN, slurm = F, slurm_options = list(),
                        n_nodes = 1, n_cores = 1, jobname = basename(getwd()), 
                        ...) {
  
  if (!slurm && n_nodes > 1) {
    stop('n_nodes > 1 but but slurm is disabled. ',
         'Did you mean to set slurm = T in main script?')
  }
  ...
  if (slurm) {
    ...
    load_libs('rslurm')
    sjob <- rslurm::slurm_apply(FUN, Y, jobname = jobname, pkgs = 'base',
