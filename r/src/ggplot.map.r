...
ggplot.map <- function(map = c('black','ggmap'), maptype = 'roadmap', 
  ...
  if (map == 'ggmap') {
    ...
    sitemap <- get_map(location = c(lon = center.lon, lat = center.lat), 
                       scale = 2, zoom = zoom, maptype = maptype, 
                       color = color, crop = FALSE)
    ...
    m1 <- ggmap(sitemap) + coord_cartesian() + theme_bw() + 
          labs(x = 'LONGITUDE', y = 'LATITUDE')
    return(list(map = m1, shift.lat = shift.lat, shift.lon = shift.lon))
  } # end if ggmap
