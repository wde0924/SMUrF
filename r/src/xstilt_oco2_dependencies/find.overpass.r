...
# dlat, dlon for lat,lon from site center
...
find.overpass <- function(date.range, lon.lat, oco2.ver = c('b7rb','b8r', 'b9r'),
  ...
  for (f in 1:length(oco2.file)) {
    ...
    if (tot.count > 0) {
      ...
      # also search for soundings near site center,
      if (urbanTF) {
        urban.dat <- obs %>% filter(lon >= (lon.lat$sitelon - dlon),
                                    lon <= (lon.lat$sitelon + dlon),
                                    lat >= (lon.lat$sitelat - dlat),
                                    lat <= (lon.lat$sitelat + dlat))
