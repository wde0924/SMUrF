...
#' @param:
#' urbanTF, dlon, dlat: for find.overpass(), e.g., site.lat +/- dlat
...
get.site.track <- function(site, oco2.ver, oco2.path, searchTF = F,
                           ...
                           lon.lat = get.lon.lat(site, dlat = 1.5, dlon = 1),  # 2x3 deg 
                           urbanTF = T, dlon.urban = 0.5, dlat.urban = 0.5,
                           thred.count.per.deg.urban = 50, txtpath, rmTF = F){

  library(dplyr)

  # instead of inputting all info mannually, use find.overpass() to find
  # overpasses given a site lat.lon
  ...
  # at least one sounding near the site
  if (urbanTF) {
